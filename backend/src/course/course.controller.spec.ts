...
const CourseMockService = {
  ...
  update: jest.fn().mockImplementation((id: string, updateCourseDto: UpdateCourseDto) => {
    return {
      id,
      ...updateCourseDto,
    };
...
const ContentMockService = {
  save: jest.fn().mockImplementation((id: string, createContentDto: CreateContentDto) => {
    return {
      id: 'testid',
      dateCreated: new Date(),
      ...createContentDto,
  ...
  update: jest
    .fn()
    .mockImplementation((id: string, contentId: string, updateContentDto: UpdateContentDto) => {
      return {
        id: contentId,
        ...updateContentDto,
      };
    }),
  delete: jest.fn().mockImplementation((id: string, contentId: string) => contentId),
};
...
describe('CourseController', () => {
  ...
  describe('saveContent', () => {
    it('should get the saved content', async () => {
      const spy = jest.spyOn(global, 'Date');
      const content = await controller.saveContent('testcourseid', undefined, {
        name: 'test',
        description: 'test',
      });
      const date = spy.mock.instances[0];
  ...
  describe('findAllContentsByCourseId', () => {
    it('should get the array of contents', async () => {
      const contents = await controller.findAllContentsByCourseId('testcourseid', {});
  ...
  describe('updateContent', () => {
    it('should update a content and return changed values', async () => {
      const updatedContent = await controller.updateContent('testid', 'testcontentid', undefined, {
        name: 'test',
        description: 'test',
      });
